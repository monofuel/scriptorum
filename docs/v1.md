...
- [ ] `IT-01` Agent-to-queue integration with fixture harness. Test: fake codex output containing `submit_pr("...")` creates exactly one queue file with ticket/branch/worktree/summary metadata.
- [ ] `IT-02` Queue success end-to-end with real branch merge. Test: passing fixture processes one queue item, moves ticket `in-progress -> done`, removes queue entry, and makes ticket commit reachable from `master`.
- [ ] `IT-03` Queue failure end-to-end with test failure. Test: failing fixture processes one queue item, moves ticket `in-progress -> open`, removes queue entry, and appends failure note text.
- [ ] `IT-04` Single-flight queue ordering with two pending items. Test: one processor invocation completes only the oldest item and leaves the second item pending.
- [ ] `IT-05` Merge conflict handling path. Test: conflicting `master` change causes merge step failure, reopens ticket, and records merge failure output.
- [ ] `IT-06` Orchestrator tick ordering integration. Test: `runOrchestratorForTicks` processes merge queue before assigning additional open tickets.
